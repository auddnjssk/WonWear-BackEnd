...
public class CategoryController {
	...
	private Constants constants;
 	...
 	public ResponseEntity<?> getMenu(HttpServletRequest request,HttpSession session) throws Exception {
    	...
    	List<Map<String, Object>> selectResponse = categoryService.getCategory();
    ...
    public ResponseEntity<?> createMenu(@RequestBody MenuRequest requestBody) throws IOException {
 		...
 		ResponseEntity<String> selectResponse = categoryService.createCategory(requestBody);
 	...
 	@DeleteMapping("/category/{cate_no}")
 	@Operation(summary = "카테고리 삭제", description = "카테고리 삭제")
 	public ResponseEntity<?> deleteCategory(@PathVariable Long cate_no) throws IOException {
 		
 		ResponseEntity<String> selectResponse = categoryService.deleteCategory(cate_no);
...
// 	
// 		ResponseEntity<String> response = categoryService.createSubMenu(requestBody);
// 		
// 		// 응답 DTO에 jwtToken 포함
...
// 		
// 		ResponseEntity<String> response = categoryService.deleteSubMenu(mainmenuId,submenuId);
// 		
// 		// 응답 DTO에 jwtToken 포함
// 		if(response.getStatusCode() == HttpStatus.OK) {
// 			ResponseDTO responseDTO = new ResponseDTO.Builder()
